...
class SeleniumMiddleware(object):
    ...
    def process_request(cls, request, spider):
        if spider.name == 'youmin':

            # selenium + PhantomJS 获取动态网页
            if spider.browser:
                ...
                if spider.isfirst:
                    ##等待下一页加载完成点击
                    WebDriverWait(spider.browser, 10).until(lambda x: x.find_element_by_link_text('下一页')).click()
                else:
        ...
        if spider.name == 'game_describe':
            ...
            time.sleep(2)
            try:
                button = driver.find_element_by_xpath("//a[@class ='zhanbtn']")
                button.click()
            except:
                pass
            time.sleep(0.5)
